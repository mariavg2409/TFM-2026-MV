...
class AdapterMLP(nn.Module):
    """MLP that projects between the dimension of pretrained embeddings and n_embd."""
 
    def __init__(self, in_dim, out_dim, n_layers, n_hidden):
        super().__init__()
        assert n_layers >= 1
        dims = [in_dim] + [n_hidden] * (n_layers - 1) + [out_dim]
        layers = []
...
class DelphiConfig:
    ...
    pretrained_embeddings: Path | str = None
    pretrained_embd_dim: int = None 
    adapter_n_layers: int = 2          # number of layers in MLP_enc and MLP_dec
    adapter_n_hidden: int = 256        # neurons per hidden layer in the MLPs
...
class Delphi(nn.Module):
    ...
    def __init__(self, config, pretrained_embeddings=None):
        ...
        if config.pretrained_embeddings is not None:
            # cargar embeddings preentrenados y congelarlos
            self.transformer.wte.weight = nn.Parameter(
                torch.tensor(pretrained_embeddings, dtype=torch.float32), requires_grad=False
            )
            # MLP_enc: pretrained_embd_dim → n_embd
            self.mlp_enc = AdapterMLP(config.pretrained_embd_dim, config.n_embd,
                                      config.adapter_n_layers, config.adapter_n_hidden)
            # MLP_dec: n_embd → pretrained_embd_dim (para weight tying con E)
            self.mlp_dec = AdapterMLP(config.n_embd, config.pretrained_embd_dim,
                                      config.adapter_n_layers, config.adapter_n_hidden)
            # no hay lm_head separado: logits = MLP_dec(h) @ E^T
        else:
            self.lm_head = nn.Linear(config.n_embd, config.vocab_size, bias=False)
            # weight tying estándar
    ...
    def forward(self, idx, age, targets=None, targets_age=None, validation_loss_mode=False):
        ...
        if self.config.pretrained_embeddings is not None:
            tok_emb = self.mlp_enc(tok_emb)  # (b, t, n_embd)
        ...
        if targets is not None:
            if self.config.pretrained_embeddings is not None:
                # u = MLP_dec(h), logits = u @ E^T
                u = self.mlp_dec(x)                              # (b, t, pretrained_embd_dim)
                logits = u @ self.transformer.wte.weight.T       # (b, t, vocab_size)
            else:
            ...
            if self.config.pretrained_embeddings is not None:
                u = self.mlp_dec(x)
                logits = u @ self.transformer.wte.weight.T
            else:
                logits = self.lm_head(x[:, :, :])
            loss = None
        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                if not p.requires_grad:
                    continue  # saltar parámetros congelados (e.g. embeddings preentrenados)
